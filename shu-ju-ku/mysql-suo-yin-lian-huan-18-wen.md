...
| id | select\_type | table  | partitions | type  | possible\_keys | key     | key\_len | ref               | filtered | rows | Extra |
| -- | ------------ | ------ | ---------- | ----- | -------------- | ------- | -------- | ----------------- | -------- | ---- | ----- |
...
* select\_type：select关键字对应的那个查询的类型，如SIMPLE,PRIMARY,SUBQUERY,DEPENDENT,SNION 。
* table：每个查询对应的表名 。
...
索引名index\_name可自己命名，缺省时，MySQL将根据第一个索引列赋一个名称。另外，ALTER TABLE允许在单个语句中更改多个表，因此可以在同时创建多个索引。 3、 使用CREATE INDEX命令创建。
